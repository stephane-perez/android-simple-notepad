...
> in `CryptoManager.kt`, across two rounds. The header-authentication, key-separation,
> size limit, and backup fixes came out of the first round; the second round caught two
> real remaining gaps — the 100 KB limit wasn't actually enforced *inside*
> `CryptoManager.decrypt()` itself (only by its one caller), and decrypted bytes weren't
> strictly validated as UTF-8 (silent `�` replacement could mask corruption as garbled
> text) — both fixed. Two follow-up suggestions were declined as disproportionate for a
> personal notes app: a key-generation/format version byte (nothing to version yet — no
> second key or algorithm exists), and anti-rollback protection against an attacker
> replacing a file with an older, still-authentic version of the same note (would need a
> separately-tracked, protected counter for a threat that, worst case, shows you a stale
> draft).
